...
Original file is located at
    https://colab.research.google.com/drive/1BZjgPbMy-4APbCGJxxiHO3K_IhX9ov9k
"""
...
!gdown --id 14Qk5sdJfOz8BS_EcL6lQ0yPKucDKcfor

#unzip train

import zipfile
zip_ref = zipfile.ZipFile('/content/train.zip') #for teeth
...
!gdown --id 1PBlHPaCSjz1J9k_unvZhe7gk_DjoBOnj

#unzip test


zip_ref = zipfile.ZipFile('/content/test.zip') #for teeth
zip_ref.extractall('/content')
...
# read,resize dataset for train,test
...
datasetObjectt = readDataset('/content/train/images',
                            '/content/train/masks')  #for teeth tai train,test
datasetObjectt.readPathes()

datasetObject = readDataset('/content/test/images',
                            '/content/test/masks')
...
# blending the original x with the output of a small convolutional operation (1x1 kernel) using an average function.
# This blending process allows the model to consider both the original features (x) and a modified version of the features through the convolution, promoting richer representations
#this is actual resnet
#sum thake avg best result dei,so avg use kora
...
def resunet(input, filter, padding, strides, kernel):
  ...
  resunet = models.Model(inputs = input, outputs = output, name = 'RESNET')
  resunet.summary()
  return resunet

R= resunet(input = layers.Input(shape = (224, 224, 1)), filter = 32,
              padding = 'same', kernel = 3, strides = 1)

tf.keras.utils.plot_model(R, show_shapes = True)

#epochs result print

def samples(resunet, images, realMasks):
  masks = tf.squeeze(resunet.predict(images))
...
for epoch in range(epochs):
    ...
    predictMasks = tf.squeeze(R.predict([realImages], verbose = 0))
    ...
    if epoch % 500 == 0:
        ...
        samples(R, validImages[validIndexs], validMasks[validIndexs])
...
R.compile(loss = tf.keras.losses.BinaryFocalCrossentropy(),
...
#resnet accuracy,precision history

history = R.fit(trainImages, trainMasks, epochs = 80, batch_size = 4,
...
result=R.evaluate(validImages, validMasks)
...
result=R.evaluate(trainImages, trainMasks)
...
masks_pred = R.predict(validImages)
...
def save_predicted_masks(images, y_pred, save_folder='/content/drive/My Drive/predicted_masks', dpi=300):
    ...
    for i in range(num_images):
        ...
        save_path = os.path.join(save_folder, f'predicted_teeth{i + 1}.png')
        plt.savefig(save_path, dpi=dpi)
        plt.close()

    print(f"Predicted masks saved in {save_folder} folder.")

save_predicted_masks(validImages,masks_pred)

masks_pred = R.predict(trainImages)
masks_pred = (masks_pred >= 0.5).astype('int')
...
def save_predicted_mask(images, y_pred, save_folder='/content/drive/My Drive/predicted_masks', dpi=300):
...
save_predicted_mask(trainImages,masks_pred)